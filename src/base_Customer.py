...
class Item:
    ...
    def __init__(self, id_customer, id_order, id_patient, id_item):
...
class Patient:
    ...
    Attributes:
        id_customer: ID of the customer this patient belongs to
    ...
    def __init__(self, id_customer, id_order, id_patient):
        """
        ...
        self.list_items = self._create_items_for_patient(
            self.id_customer, self.id_order, self.id_patient, self.num_items)

    def _create_items_for_patient(self, id_customer, id_order, id_patient, num_items):
        """Create items for a patient"""
        items = []
        for _ in range(num_items):
            item_id = self._get_next_item_id()
            item = Item(id_customer, id_order, id_patient, item_id)
            items.append(item)
        return items
...
class Order:
    ...
    Attributes:
        ...
        completed_patient_count: Counter for completed patients
        makespan: Makespan of this order
    """

    def __init__(self,  id_customer, id_order):
        """
        ...
        self.completed_patient_count = 0
        self.makespan = None

        # Create patients for this order using the provided function
        self.list_patients = self._create_patients_for_order(
            self.id_customer, self.id_order, self.num_patients)

    def _create_patients_for_order(self, id_customer, id_order, num_patients):
        """Create patients for an order"""
        patients = []
        for _ in range(num_patients):
            patient_id = self._get_next_patient_id()
            patient = Patient(id_customer, id_order, patient_id)
...
class Customer:
    ...
    def create_order(self):
        ...
        while True:
            ...
            order = Order(self.id_customer, order_id)
